...
<div class="page-header">
    <h2><i class="bi bi-person-badge"></i> Designation Type</h2>
    <button class="btn-premium" onclick="openAddModal()">
        <i class="bi bi-plus-lg"></i> Add New Designation Type
        </button>
</div>

<!-- Designation Type Form Card -->
<div class="glass-card" id="form-card" style="display: none;">
    <h5 class="card-title">
        <i class="bi bi-pencil-square"></i>
        <span id="form-title">Add New</span>
    </h5>
    ...
    <form id="designation-form">
        ...
        <div class="row">
            <div class="col-md-6">
                <div class="form-group">
                    <label class="form-label">
                        <i class="bi bi-type"></i>  नाम <span class="required">*</span>
                    </label>
                    <input type="text" id="designation-name" class="form-control-premium" placeholder="Designation Type का नाम दर्ज करें">
            ...
            <div class="col-md-6">
                <div class="form-group">
                    <label class="form-label">
                        <i class="bi bi-tag"></i> प्रकार <span class="required">*</span>
                    </label>
                    <select id="designation-type" class="form-control-premium">
                        ...
                        <option value="for_all">सभी के लिए</option>
                    </select>
                    <div id="type-error" class="error-message"><i class="bi bi-exclamation-circle"></i> <span></span></div>
                </div>
            </div>
        </div>
        ...
        <div style="display: flex; gap: 14px; margin-top: 12px;">
            <button type="submit" class="btn-premium" id="submit-btn">
                <i class="bi bi-check-lg"></i> <span id="submit-text">Designation Typeजोड़ें</span>
            </button>
...
<div class="glass-card">
    ...
    <div id="empty-state" class="empty-state" style="display: none;">
        ...
        <h3>कोई Desiggnation Type नहीं मिला</h3>
        <p>नया Designation Type जोड़ने के लिए ऊपर बटन पर क्लिक करें</p>
    </div>
</div>
...
<script>
    const API_BASE = '/api/designation-types';
    let deleteModal;
    ...
    async function loadDesignationTypes() {
        try {
            ...
            if (result.success) {
                renderDesignationTypes(result.data);
            } else {
    ...
    // Render designation types in table
    function renderDesignationTypes(designationTypes) {
        ...
        tbody.innerHTML = designationTypes.map((item, index) => `
            <tr>
                <td><strong>${index + 1}</strong></td>
                <td style="font-weight: 600;">${item.name}</td>
                <td>
                    <span class="badge-premium ${item.type}">
                ...
                <td>
                    <button class="action-btn edit" onclick="editDesignationType(${item.id})" title="संपादित करें">
                        <i class="bi bi-pencil-fill"></i>
                    </button>
                    <button class="action-btn delete" onclick="deleteDesignationType(${item.id}, '${item.name}')" title="हटाएं">
    ...
    function openAddModal() {
        ...
        document.getElementById('form-title').textContent = 'नया Designation Type जोड़ें';
        document.getElementById('submit-text').textContent = 'Designation Type जोड़ें';
        document.getElementById('designation-id').value = '';
        document.getElementById('designation-name').value = '';
        document.getElementById('designation-type').value = '';
    ...
    async function editDesignationType(id) {
        showLoading();
        try {
            ...
            if (result.success) {
                ...
                clearErrors();
                document.getElementById('form-card').style.display = 'block';
                document.getElementById('form-title').textContent = 'Designation Type संपादित करें';
                ...
                document.getElementById('designation-type').value = data.type;
    ...
    async function handleFormSubmit(e) {
        ...
        const type = document.getElementById('designation-type').value;
        
        // Client-side validation
        let hasError = false;
        
        if (!name) {
            showFieldError('designation-name', 'Designation Type का नाम आवश्यक है।');
            hasError = true;
        } else if (name.length > 255) {
            showFieldError('designation-name', 'Designation Type का नाम 255 characters से अधिक नहीं हो सकता।');
            hasError = true;
        }
        
        if (!type) {
            showFieldError('designation-type', 'प्रकार चुनना आवश्यक है।');
            hasError = true;
        }
    ...
    // Delete designation type
    function deleteDesignationType(id, name) {
